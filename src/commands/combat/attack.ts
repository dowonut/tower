...
export default {
  ...
  async execute(
    ...
    // Perform attack
      ...
      await performAttack({ player, target, enemies, attackName });
    }
  },
} as Command;

/** Perform an attack. */
async function performAttack(args: { player: Player; target: Enemy; enemies: Enemy[]; attackName: string }) {
  ...
  for (let attackDamage of attack.damage) {
    ...
    const evaluatedDamageInstance = await game.evaluateDamageInstance({ damage: attackDamage, source: player });
