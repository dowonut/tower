...
const player = {
    ...
    getPassives: async function (target) {
        ...
        for (const passive of passives) {
            ...
            if (passive.name !== "all" && passive.name !== weaponType)
                continue;
    ...
    getAttack: async function (attackName) {
        ...
        if (!attackRef[0])
            return undefined;
        const attackData = attacks.find((x) => x.name == attackRef[0].name.toLowerCase());
    ...
    getAttacks: async function () {
        ...
        for (const playerAttack of playerAttacks) {
            const attackData = attacks.find((x) => x.name == playerAttack.name.toLowerCase());
            if (!attackData)
                continue;
    ...
    giveRandomLoot: async function (message, server) {
        ...
        return await game.reply(message, `you explore the **${regionName}** and find ${quantityText}**${itemName}** ${itemEmoji}`);
    },
    // Give xp to player
    giveXp: async function (xp, message, server) {
        ...
        if (levelUp > 0) {
            return await game.reply(message, `**Level Up!**

:tada: You are now level \`${player.level}\`
:low_brightness: You have \`${levelUp}\` new stat points`, true, config.green, config.emojis.level_up);
        }
        else
            return undefined;
    },
    ...
    getUnlockedMerchants: async function () {
        ...
        for (const merchantRef of merchantsRef) {
            const merchant = merchants.find((x) => x.category == merchantRef.category && x.floor == this.floor);
            if (!merchant)
                continue;
    ...
    unlockRandomMerchant: async function (message, server) {
        ...
        const reply = await game.reply(message, `you explore the **${regionName}** and come across **${mName}**, the local \`${mCategory}\``);
